...
**tl;dr** - Istio is **not** a CNI plugin, the [Istio CNI](https://istio.io/latest/docs/setup/additional-setup/cni/) just sets up iptables redirection which forces all pod traffic through the sidecar proxy.
:::
...
For some users however this is a problem because it means whoever or whaetver is putting pods into their cluster requires some highly [privileged capabilities](https://kubernetes.io/docs/tasks/configure-pod-container/security-context/). The CNI solves that problem for those users. Instead of giving cluster users the ability to create highly privilged pods, the cluster admins deploy the Istio CNI and give _it_ the necessary capabilities to set up iptables redirection. Now all their users have to do is create a pod with regular privileges and the Istio CNI handles the rest.
